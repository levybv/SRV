...
/**
 * Action para tratar as requisições de Upload de Agrupamento de Filiais
 * 
 * @author Walter Fontes
 */
public class UploadAgrupaFilialServlet extends HttpServlet {
	
	//Log4J
	private static final Logger log = Logger.getLogger(UploadAgrupaFilialServlet.class);    
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if ((contentType != null) && (contentType.indexOf("multipart/form-data") >= 0)) {
				...
				while (it.hasNext()){
					...
					if (!fileItem.isFormField()){
				        ...
				        int linha = 0;
				        List agrupamentos = new ArrayList();
				        for(String line=br.readLine(); line!=null; line=br.readLine()) {
				        	
				        	linha++;
				        	
				        	if (ObjectHelper.isNotEmpty(line)) { 
				        		log.debug("\nAgrupamento de Filial => Linha: " + line);
				        		...
				        		try {
					        		...
					        		salarioBaseVO.setIdFilial(new Integer(st.nextToken()));
				        			...
				        			agrupamentos.add(salarioBaseVO);
				        ...
				        List errosAlteracao = SalarioBaseBusiness.getInstance().importaAgrupamentoFilias(agrupamentos);
			...
			request.getSession().setAttribute("erros", erros);
			log.error("Ocorreu erro ao importar agrupamentos de filiais.", e);			
		}
		
		String destino = "/srv/agrupaFilial.do?operacao=inicio";
