		...
		try {
			...
			if (listaCampanha.size() == 1) {
				...
				if ( RelatorioCampanhaBusiness.getInstance().isTabelaExist(ano, mes, tabelaConsulta) ) {
					...
					subTitulo.put("NOME", "Relatório de Campanha");
					...
					setMensagemErro(pRequest, "Período não existe para a Campanha selecionada.");
					return  pMapping.findForward(RelatorioCampanhaAction.FORWARD_RELATORIO_CAMPANHA);
				}
			} else {
				setMensagemErro(pRequest, "Erro ao gerar o relatório de Campanha.");
				return  pMapping.findForward(RelatorioCampanhaAction.FORWARD_RELATORIO_CAMPANHA);
			}
			...
			throw new ApresentacaoException(log, "Não foi possível gerar relatório: Campanha em excel ou csv.", e);
		} catch (Exception e) {
			throw new ApresentacaoException(log, "Não foi possível gerar relatório: Campanha em excel ou csv.", e);
		}
