 ...
 *
 */
public class RelatorioBonusDesempenhoColaboradorAction extends BasicAction {

	private static final String FORWARD_RELATORIO_BONUS_DESEMPENHO_COLABORADOR = "relatorio";
	...
	public ActionForward montaTelaPrincipal(ActionMapping pMapping, ActionForm pForm, HttpServletRequest pRequest, HttpServletResponse pResponse) throws SRVException {
		...
		if (periodos != null) {
			...
			listaRelatorio = RelatorioGenericBusiness.getInstance().pesquisaRelatorioBonusDesempenho(status, mes, ano, false);
			pRequest.setAttribute("listaRelatorioBonus", listaRelatorio);
		} else {
			listaRelatorio = RelatorioGenericBusiness.getInstance().pesquisaRelatorioBonusDesempenho(status, null, null, false);
			pRequest.setAttribute("listaRelatorioBonus", listaRelatorio);
		}

		return pMapping.findForward(FORWARD_RELATORIO_BONUS_DESEMPENHO_COLABORADOR);
	}
	...
	public ActionForward exportaRelatorioExcel(ActionMapping pMapping, ActionForm pForm, HttpServletRequest pRequest, HttpServletResponse pResponse) throws SRVException {
		...
		try {
			...
			tipoPesquisaVO.setCodigo(Constantes.RELATORIO_TIPO_BONUS_DESEMPENHO_POR_COLABORADOR);
