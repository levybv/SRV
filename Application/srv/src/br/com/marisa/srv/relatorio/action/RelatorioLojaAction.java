 ...
 *
 */
public class RelatorioLojaAction extends BasicAction {

	private static final String FORWARD_RELATORIO_LOJA = "relatorio";
	...
	public ActionForward inicio(ActionMapping pMapping, ActionForm pForm, HttpServletRequest pRequest, HttpServletResponse pResponse) throws Exception {
		...
		relatorioTipoVO.setCodigo(Constantes.RELATORIO_TIPO_LOJA);
		pesquisaVO.setRelatorioTipoVO(relatorioTipoVO);
		pesquisaVO.setIsAtivo(true);
		pRequest.setAttribute("listaRelatorioLoja", RelatorioBusiness.getInstance().obtemRelatorio(pesquisaVO));
		return pMapping.findForward(FORWARD_RELATORIO_LOJA);
	}
	 ...
	 * @throws SRVException
	 */
	public ActionForward geraRelatorioLoja(ActionMapping pMapping, ActionForm pForm, HttpServletRequest pRequest, HttpServletResponse pResponse) throws SRVException {
		...
		try {
			...
			throw new SRVException("Erro ao gerar o relatorio de loja", ex);
		}
