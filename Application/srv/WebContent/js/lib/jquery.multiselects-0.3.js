...
jQuery.fn.multiSelect = function(to, options) {
	...
	options = $.extend({
		...
		sortOptions: false,	// true => sort options to keep them in the same order
	...
	function moveOptions(from, toSel, beforeMove, afterMove, sortOptions, moveAll) {
		...
		// sort both selects
		// Trecho do código que não está funcionando conforme esperado (Detectado por Petrônio Braga) 
		//if(sortOptions) {
			//from.sortOptions();
			//$(toSel).sortOptions();
		//}
