	...
	<div class="boxCenter">
		...
		<div class="panel">
			<form id="form" name="form" method="post" action="filial.do?operacao=inicio" accept-charset="ISO-8859-1">
								...
								<td class="componente">
									...
									<select id="idDescrTipoFil" name="idDescrTipoFil" class="campo">
									   ...
									   <c:forEach var="filial" items="${listaTipoFil}" varStatus="id">
                                    	  <option value="${filial.codTpFilial}"><c:out value="${filial.descricao}"/></option>
                                       </c:forEach>
                                    </select>
                                </td> 
				...
				<input id="botaoIncluir" type="button" class="button" value="Incluir" style="float:right" />
				<input id="atualizar" name="atualizar" type="button" class="buttonAtualizar" value="Alterar" />
				<br/></div>
				
				<display:table pagesize='10' id="listaFiliais" name="listaFiliais" class="table" requestURI="">     
					<display:setProperty name="paging.banner.placement" value="bottom" />   
					<display:column title=""  style="text-align:center;"><input type="radio" name="checkbox" id="checkbox" value="${listaFiliais.codFilial}@${listaFiliais.codEmpresa}@${listaFiliais.descricao}@${listaFiliais.flagAtivo}@${listaFiliais.cnpjStr}@${listaFiliais.descricaoTpFil}@${listaFiliais.flagMeta100}@${listaFiliais.codTpFilial}@${listaFiliais.dataStrInauguracao}"></display:column> 					
					...
					<display:column property="descricao"       title="Descri&ccedil;&atilde;o" style="cursor: pointer; width: 30%; vertical-align: middle;"/>             
					<display:column property="flagAtivoStr"    title="Ativa"                   style="text-align:center; cursor: pointer; width: 15%; vertical-align: middle;"/>             					
					<display:column property="cnpjStr"         title="CNPJ"                    style="text-align:center; cursor: pointer; width: 25%; vertical-align: middle;"/>
					<display:column property="descricaoTpFil"  title="Tipo Filial"             style="text-align:center; cursor: pointer; width: 30%; vertical-align: middle;"/>
					<display:column property="dataInauguracao"  title="Inauguração"            format="{0,date,dd/MM/yyyy}" style="text-align:center; cursor: pointer; width: 30%; vertical-align: middle;"/>
				</display:table>
				<BR>
