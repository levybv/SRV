	...
	$(document).ready(function() {
		...
		EscalaBusinessAjax.obtemEscala(idEscala,
			function (escalaVO) {
				if (escalaVO != null) {
					...
					$("#descricaoES").val(escalaVO.descricaoEscala);

					if (escalaVO.percentual) {
						$("input:radio[name=percentualES][value=S]").attr('checked', true);
					} else {
						$("input:radio[name=percentualES][value=N]").attr('checked', true);
					}

					if (escalaVO.flgAplicaFxResultNoRealz == 'S') {
						$("input:radio[name=aplEscalaRelz][value=S]").attr('checked', true);
					} else {
						$("input:radio[name=aplEscalaRelz][value=N]").attr('checked', true);
					}
					
					if(escalaVO.numEscala != null){
					    $("#nrEscalaES").val(escalaVO.numEscala);
					}else{
					   	$("#nrEscalaES").val("");
					}
					
					$("#limiteES").val(escalaVO.limiteFormatado);
						...
						function (indicadores) {
							if (indicadores != null) {
								...
								if (escalaVO.idIndicador != null) {
									$("#idIndicadorES").val(escalaVO.idIndicador);
									$("input:radio[name=associacao][value=1]").attr('checked', true);
						...
						function (grupos) {
							if (grupos != null) {
								...
								if (escalaVO.idGrupoIndicador != null) {
									$("#idGrupoIndicadorES").val(escalaVO.idGrupoIndicador);
									$("input:radio[name=associacao][value=2]").attr('checked', true);
						...
						function (grupos) {
							if (grupos != null) {
								...
								if (escalaVO.idGrupoRemVar != null) {
									$("#idGrupoRemVarES").val(escalaVO.idGrupoRemVar);
									$("input:radio[name=associacao][value=3]").attr('checked', true);
		...
		$("#detMeta").validate({
	         ...
	         submitHandler : function(form){
			        $("#div-botao").hide();  
		           	$("#div-load").show(); 
			       	document.detMeta.action = "escala.do?operacao=alteraEscala";   
		...
		<table class="tabelaComponente">
			<tbody> 
				...
				<tr>
					...
					<td class="componente" >
						<input type="radio" id="associacao" name="associacao" value="1" onClick="selecionaAssociacao()"/>
						<select id="idIndicadorES" name="idIndicadorES" class="campo" disabled>
				...
				<tr>
					...
					<td class="componente" >
						<input type="text" id="limiteES" name="limiteES" value="" maxlength="8" size="10" class="campo2" />
					</td>
				</tr>
				...
				<tr>
					...
					<td class="componente" >
						<input type="text" id="nrEscalaES" name="nrEscalaES" value="" maxlength="6" size="10" class="campo2" />
					</td>
				</tr>
