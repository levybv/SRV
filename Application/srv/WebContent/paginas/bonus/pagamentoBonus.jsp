...
<script type="text/javascript" src="js/ms/srv.bonus.js" charset="ISO-8859-1"></script>
...
<div id="box" class="box">
	<div class="boxCenter">
		<div class="barTitleInternoCadastro">Relatório Pagamento de Bonus</div>
		<br/>
		<div class="panel">
			<form id="form" name="form" method="post" action="allPageBonus.do" accept-charset="ISO-8859-1">
                    	...
                    	<tbody>
                        	<tr>
 	                        	<td class="componente" width="5%">
 	                        		<label class="label" for="filialDesc">Nº ano:</label>
                                </td>
                                <td width="20px">
 	                        		<select id="ano" name="ano" class="campo">
                                    	<option value="" selected>[SELECIONE]</option>
                                <c:forEach var="item" items="${listaAno}" >
									<option value='<c:out value="${item.id}" />'><c:out value="${item.ano}" /></option>
								</c:forEach>
                                    </select>
                                </td>
                            </tr>
                        	<tr>
 	                        	<td class="componente" width="5%">
 	                        		<label class="label" for="filialDesc">Nome mês:</label>
                                </td>
                                <td width="20px" >
 	                        	<select id="mes" name="mes" class="campo">
                                    	<option value="" selected>[SELECIONE]</option>
                                 <c:forEach var="item" items="${listaMes}" >
									<option value='<c:out value="${item.id}" />'><c:out value="${item.mesStr}" /></option>
								</c:forEach>
                                    </select>
                                </td>
                            </tr>   
