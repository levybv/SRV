...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		IndicadorBusinessAjax.obtemIndicador(idIndicador,
		
			function (indicadorVO) {
			
				$("#idIndicadorID").val(indicadorVO.idIndicador);
				
				$("#descricaoID").val(indicadorVO.descricaoIndicador);
				if (indicadorVO.formulaConceito != null) {
					$("#formulaConceitoID").val(indicadorVO.formulaConceito);
				}
				if (indicadorVO.formulaIndicador != null) {
					$("#formulaID").val(indicadorVO.formulaIndicador);
				}
				
				if (indicadorVO.verbaRH != null) {
					$("#verbaRHID").val(indicadorVO.verbaRH);
				}

				if (indicadorVO.ativo != null) {
					$("#ativoID").val(indicadorVO.ativo);
				}

				if (indicadorVO.flgSentido != null) {
					$("#sentidoIndicadorID").val(indicadorVO.flgSentido);
				}

				if (indicadorVO.flgPreenchAtingIgualRealz == "S") {
					$("#flg_Preench_Ating_igual_RealzID").attr('checked','checked');
				}
				...
				GrupoIndicadorBusinessAjax.obtemGruposIndicadoresCorporativos(
					function (grupos) {
						if (grupos != null) {
							...
							$("#idGrupoID").val(indicadorVO.idGrupoIndicador);
						}				
				});	
		    	...
		    	EscalaBusinessAjax.obtemEscalas(
					function (escalas) {
						if (escalas != null) {
							...
							if (indicadorVO.idEscala != null) {
								$('#idEscalaID').val(indicadorVO.idEscala);
							}
						}
				});							
				...
				IndicadorBusinessAjax.obtemListaFonte(function (listaFonte) {
					if (listaFonte != null) {
						...
						if (indicadorVO.descricaoFonte != null) {
							$('#fonteID').val(indicadorVO.descricaoFonte);
						}
					}
		    	});
				...
				IndicadorBusinessAjax.obtemListaDiretoria(function (listaDiretoria) {
					if (listaDiretoria != null) {
						...
						if (indicadorVO.descricaoDiretoria != null) {
							$('#diretoriaID').val(indicadorVO.descricaoDiretoria);
						}
					}
		    	});
		...
		$("#detIndicador").validate({
	    	rules: {
	         ...
	         submitHandler : function(form){
		        $("#div-botao").hide();  
	           	$("#div-load").show(); 
		       	document.detIndicador.action = "indicador.do?operacao=alteraIndicador";   
		...
		<table class="tabelaComponente">
			<tbody> 
				...
				<tr>
					...
					<td class="componente" >
						<textarea rows="4" class="campo2" cols="80" id="formulaConceitoID" name="formulaConceitoID"></textarea>
