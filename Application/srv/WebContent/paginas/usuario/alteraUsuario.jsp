		...
		$("#detFilial").validate({
	    	rules: {
	         ...
	         submitHandler : function(form){
		        $("#div-botao").hide();  
	           	$("#div-load").show(); 
		       	document.detFilial.action = "usuario.do?operacao=alteraUsuario";
			...
			function (perfis) {
				if (perfis != null) {
					...
					for (var i=0; i<perfis.length; i++) {
						perfilVO = perfis[i];
						options[options.length] = new Option(perfilVO.descricao, perfilVO.idPerfil, true, true);
					}
				}
				
				UsuarioBusinessAjax.obtemUsuario(idUsuario,
					function (usuarioVO) {
						$("#idUsuarioM").val(usuarioVO.idUsuario);
						$("#nomeM").val(usuarioVO.nome);
						$("#matriculaM").val(usuarioVO.matricula==null?"":usuarioVO.matricula);
						if (usuarioVO.ativo) {
							$("#ativoM")[0].checked = true;
						}
						if (usuarioVO.autenticaAD) {
							$("#autenticaAdM")[0].checked = true;
						}
						$("#loginM").val(usuarioVO.login);
						$("#perfilM").val(usuarioVO.idPerfil);
		       }); 	
	    }); 	
    });
	...
	<form name="detFilial" id="detFilial" method="post"> 
			...
			<table class="tabelaComponente">
				<tbody> 
					...
					<tr>
						<td class="componente" >
							<label class="label">Autenticar no AD:</label> 
						</td>
						<td class="componente" >
	  						<input class="campo2" type="checkbox" id="autenticaAdM" name="autenticaAdM" />
						</td>
					</tr>
					<tr> 
						<td class="componente">
					...
					<tr>
						<td class="componente" >
							<label class="label">Reiniciar Senha:</label>
						</td>
						<td class="componente" >
							<input class="campo2" type="checkbox" id="reiniciaSenhaM" name="reiniciaSenhaM" />
						</td>
					</tr>
				</tbody>
			</table>
